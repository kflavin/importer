...
BEGIN
            DROP TABLE IF EXISTS rxvantage_dev_rk.tmp_gmdn_id_generator;
            DROP TABLE IF EXISTS rxvantage_dev_rk.tmp_gmdn_id;
            DROP TABLE IF EXISTS rxvantage_dev_rk.tmp_gmdn_id_toCompare;
            
            DROP TABLE IF EXISTS rxvantage_dev_rk.tmp_gmdn_lookup;
            DROP TABLE IF EXISTS rxvantage_dev_rk.tmp_gmdn_lookup_toCompare;
            DROP TABLE IF EXISTS rxvantage_dev_rk.tmp_gmdn_lookup_pdi_name;
            DROP TABLE IF EXISTS rxvantage_dev_rk.tmp_product_key_id_name;
        
            -- GMDN ID Tables                
            CREATE TABLE rxvantage_dev_rk.tmp_gmdn_id(
					gmdnPTName varchar(150) NOT NULL,
					PRIMARY KEY (`gmdnPTName`)
			) ENGINE=InnoDB DEFAULT CHARACTER SET=utf8mb4;
            
           CREATE TABLE rxvantage_dev_rk.tmp_gmdn_id_toCompare(
					gmdnPTName varchar(150) NOT NULL,
					PRIMARY KEY (`gmdnPTName`)
			) ENGINE=InnoDB DEFAULT CHARACTER SET=utf8mb4;
            
            CREATE TABLE rxvantage_dev_rk.tmp_gmdn_id_generator(
           ...
           CREATE TABLE rxvantage_dev_rk.tmp_gmdn_lookup_pdi_name(
            ...
            INSERT INTO tmp_gmdn_id_toCompare (gmdnPTName) SELECT Product_Name from product_keys where Product_Type="DEVICES";
            ...
            SELECT  @last_product_id := max(convert(Product_ID, UNSIGNED INT)) from product_keys where Product_Type="DEVICES";
            INSERT INTO tmp_gmdn_id_generator (id, gmdnPTName) SELECT Product_ID, Product_Name from product_keys where Product_Type="DEVICES" and Product_ID = @last_product_id;
            ...
            INSERT INTO product_keys (Product_ID, Product_Type, Product_Name) SELECT id, "DEVICES", gmdnPTName from tmp_gmdn_id_generator;
            ...
            INSERT INTO tmp_gmdn_lookup_toCompare SELECT id, PrimaryDI from gmdn_lookup; -- 7s
			...
			INSERT INTO gmdn_lookup (`id`, `PrimaryDI`) SELECT `id`, `PrimaryDI` from tmp_gmdn_lookup;
 END           
