...
BEGIN
            ...
            #CREATE INDEX idx_drug_master_Drug_ID ON drug_master (Ind_Drug_ID);
            CREATE INDEX idx_tmp_product_keys_Product_ID ON tmp_product_keys (Product_ID);
            ...
            DROP TABLE IF EXISTS tmp_NDCProduct;
			...
			FROM stage_ndc_product;

			/*---------------------------------------------------------------------------------
				Prepare tmp_stage_orangebook_product by getting TE_Code and TE_Type
			  ------------------------------------------------------------------------------- */
	
			DROP TABLE IF EXISTS tmp_stage_orangebook_product;
			
			#Create Temp Table    
			CREATE TABLE tmp_stage_orangebook_product(	
			...
			CREATE INDEX idx_tmp_1_OB_ProprietaryName ON tmp_stage_orangebook_product (ProprietaryName);
		
			INSERT INTO tmp_stage_orangebook_product(
			...
			FROM 
					tmp_product_keys k 
			LEFT JOIN 
					stage_orangebook_product o ON trade_name = ProprietaryName 
			;
            ...
            DROP TABLE IF EXISTS tmp_stage_orangebook_Ndc_product;
            
            CREATE TABLE tmp_stage_orangebook_Ndc_product(	
					ProductKey_ID 			MEDIUMINT(9),
                    Product_ID 				VARCHAR(50),
                    Client_Product_ID		VARCHAR(50),
                    ProductName				VARCHAR(400),
					ProprietaryName 		VARCHAR(400),
					NonProprietaryName  	VARCHAR(400),                    
					TE_Code 				VARCHAR(20),
					TE_Type					enum('RX','OTC','DISCN'),
                    ProductTypeName			VARCHAR(50),
                    LabelerName				VARCHAR(500),     
            ...
            CREATE INDEX tmp_NDCProduct ON tmp_NDCProduct(ProprietaryName);
            
           #drop index idx_tmp_2_OB_Prop_nonprop on tmp_stage_orangebook_product;
            
           #CREATE INDEX idx_tmp_2_OB_Prop_nonprop ON tmp_stage_orangebook_product (ProprietaryName,NonProprietaryName);
		    
            INSERT INTO tmp_stage_orangebook_Ndc_product (ProductKey_ID,LabelerName,ProductNDC,ProprietaryName,NonProprietaryName,ProductTypeName,
            ...
            FROM       
						tmp_stage_orangebook_product k
            LEFT JOIN  
						tmp_NDCProduct n ON k.ProprietaryName = n.ProprietaryName and k.NonProprietaryName = n.NonProprietaryName;
            
            CREATE INDEX idx_tmp_2_OB_Drug_ID ON tmp_stage_orangebook_Ndc_product (Product_ID);
			...
			FROM  
					tmp_stage_orangebook_Ndc_product k 			   
			LEFT JOIN  
					stage_indications i ON i.Drug_id = k.Product_ID;
			
            select count(*) from tmp_drug_master;
			select count(*) from tmp_drug_master_tocompare;
             
			DELETE t.* FROM 
               tmp_drug_master t JOIN 
               tmp_drug_master_tocompare k 
			ON 
