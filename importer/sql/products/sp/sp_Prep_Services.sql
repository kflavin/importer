CREATE DEFINER=`myusername`@`%` PROCEDURE `sp_Prep_Services`()
BEGIN
    ...
    INSERT INTO tmp_product_keys_toCompare (`Product_ID`, `Product_Name`, `ProprietaryName`, `NonProprietaryName`)  
		SELECT `Product_ID`, `Product_Name`, `ProprietaryName`, `NonProprietaryName` FROM {prod}.product_keys WHERE Product_Type='SERVICES';
    ...
    INSERT INTO {prod}.product_keys (`Product_ID`, `Product_Type`, `Product_Name`, `ProprietaryName`, `NonProprietaryName`)
