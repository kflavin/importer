CREATE DEFINER=`{myusername}`@`%` PROCEDURE `sp_Prep_Device_ProdKeys_gmdn`()
BEGIN
    #--------------------------------------------------------------------------------------------*/
    # MEDICAL DEVICES
    #--------------------------------------------------------------------------------------------*/
    ...
    FROM product_device_keys_gmdn where Product_Type = 'MEDICAL DEVICES' and product_id in (select distinct Product_ID from tmp_product_keys_final);
    ...
    INSERT INTO  product_device_keys_gmdn(Product_Name,Product_ID,Product_Type,ProprietaryName,NonProprietaryName ,Eff_Date,End_Eff_Date,Created_By)
