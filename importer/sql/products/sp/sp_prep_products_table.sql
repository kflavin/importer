create definer = myusername@`%` procedure sp_prep_products_table()
BEGIN
    ...
    INSERT INTO tmp_products (`name`, `generic_name`, `rxcui_id`, `source`, `product_category_id`, `is_generic`, `approver_id`, `creator_id`, `deleter_id`, `updater_id`, created_at)
	SELECT DISTINCT t2.BrandName as Name, SUBSTRING(GROUP_CONCAT(distinct t3.STR ORDER BY t3.STR ASC SEPARATOR ', '), 1, @maxlen_generic_name) as GenericName, t2.BrandRXCUI, 'RXNORM', 1, 0, 0, 0, 0, 0, NOW() # Find all TTY=IN from RXNORM vocab and print Brand/Generic names
    ...
    UNION
	SELECT DISTINCT STR as Name, STR as GenericName, RXCUI, 'RXNORM', 1, 1, 0, 0, 0, 0, NOW() FROM stage_rxnconso WHERE SAB='RXNORM' and TTY='IN' and STR not like '%,%' ;
    ...
    INSERT INTO products (`name`, `generic_name`, `description`, `rxcui_id`, `source`, `product_category_id`, `is_generic`, `approver_id`, `creator_id`, `deleter_id`, `updater_id`, created_at)
    SELECT DISTINCT `name`, `generic_name`, `description`, `rxcui_id`, `source`, `product_category_id`, `is_generic`, `approver_id`, `creator_id`, `deleter_id`, `updater_id`, NOW() from tmp_products;
    ...
    SELECT brand_rxcui,STR,0,NOW() from stage_rxnconso t3 JOIN (
