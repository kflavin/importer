create definer = myusername@`%` procedure rxnorm_migrate2.sp_create_initial_tables()
...
#     ON t1.product_id = t2.old_id;
    ...
    CREATE TABLE tmp_company_import_product_mappings LIKE company_import_product_mappings;
    INSERT INTO tmp_company_import_product_mappings SELECT * FROM company_import_product_mappings;

    UPDATE tmp_company_import_product_mappings t1 JOIN products_to_product t2 ON t1.target_product_id = t2.old_id
    SET t1.target_product_id = t2.new_id;

    # Add back the foreign keys from the original table.  Temporarily disable checks.
    SET FOREIGN_KEY_CHECKS = 0;
    ALTER TABLE  tmp_company_import_product_mappings
        ADD CONSTRAINT `company_import_product_mappings_fk_1` FOREIGN KEY (`company_id`) REFERENCES `company` (`id`),
        ADD CONSTRAINT `company_import_product_mappings_fk_2` FOREIGN KEY (`target_product_id`) REFERENCES `products` (`id`),
        ADD CONSTRAINT `company_import_product_mappings_fk_3` FOREIGN KEY (`creator_id`) REFERENCES `user` (`id`),
        ADD CONSTRAINT `company_import_product_mappings_fk_4` FOREIGN KEY (`updater_id`) REFERENCES `user` (`id`);
    SET FOREIGN_KEY_CHECKS = 1;

    RENAME TABLE company_import_product_mappings TO deprecated_company_import_product_mappings,
								tmp_company_import_product_mappings TO company_import_product_mappings;


    SELECT 'UPDATE company_products TABLE WITH NEW PRODUCT_ID';
    DROP TABLE IF EXISTS tmp_company_products;
    CREATE TABLE tmp_company_products LIKE company_products;
    INSERT INTO tmp_company_products SELECT * FROM company_products;

    UPDATE tmp_company_products t1 JOIN products_to_product t2 ON t1.product_id = t2.old_id
    SET t1.product_id = t2.new_id;

    RENAME TABLE company_products TO deprecated_company_products,
								tmp_company_products TO company_products;


    SELECT 'UPDATE sample_request_products TABLE WITH NEW PRODUCT_ID';
    DROP TABLE IF EXISTS tmp_sample_request_products;
    CREATE TABLE tmp_sample_request_products LIKE sample_request_products;
    INSERT INTO tmp_sample_request_products SELECT * FROM sample_request_products;

    UPDATE tmp_sample_request_products t1 JOIN products_to_product t2 ON t1.product_id = t2.old_id
    SET t1.product_id = t2.new_id;

    RENAME TABLE sample_request_products TO deprecated_sample_request_products,
								tmp_sample_request_products TO sample_request_products;
