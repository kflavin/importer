...
class DeltaBaseLoader(BaseLoader):
    ...
    def _col_subset(self, compare_columns, rows):
        """
        Take a list of compare_columns, which are columns to include in the comparison.  The
        rows are a list of dicts, where keys are column names.  Return rows with a subset of columns
    ...
    def _rows_needing_updates(self, compare_columns, extra_lcols, extra_rcols, left, right):
        ...
        Return a list of rows that need updates, and include any additional columns
        from the left table that may be needed (extra_lcols)
        """
        subset_left = self._col_subset(compare_columns, left)
        subset_right = self._col_subset(compare_columns, right)
        need_updates = []

        # For rows that need to be updated
                ...
                for col in extra_lcols:
                    if col not in row:
                        try:
                            ...
                            print(f"**Could not find left[{i}]['{col}'], ensure your lcols match the table.**")
                            raise

                # extra columns to include from the right table
                for col in extra_rcols:
                    if col not in row:
                        try:
                            row[col] = right[i][col]
                        except KeyError as e:
                            print(f"**Could not find right[{i}]['{col}''], ensure your rcols match the table.**")
                            raise
        ...
        **kwargs is the keyword arguments to replace in the SQL template
        ...
        for row in rows:
            ...
            for i,column in enumerate(columns):
                if type(row) == dict:
                    if row[column]:
                        # temporarily escape, but change this to use prepared statements
    ...
    def process_found(self, found):
        # These records exist in the right table, and need to be checked to see if there are changes in the row.
        ...
        for c, batch in enumerate(super()._batcher(found)):
            ...
            logger.debug(f"RETRIEVE_RIGHT_Q:")
            right = super()._query(q, self.get_cursor(dictionary=True))
            self._perform_xform(right, self.xform_right)
            logger.debug(f"rows in right: {len(right)}")

            # Pass the rows from the left and right tables, and use "compare_columns" to check equality.
            need_updates = self._rows_needing_updates(self.compare_columns, self.extra_lcols, self.extra_rcols, left, right)

            # For rows needing updates, archive the row from the right table,
            # delete the row from the right table, and then add the new row from the left table
            ...
            if need_updates:
                ...
                # print(need_updates)
                # print("===============")
                q = self._build_and_or_query(self.ARCHIVE_Q, 
                                self.join_columns, 
                                need_updates,
                                table_name=self.right_table_name,
                                archive_table_name=self.right_table_name_archive)
                logger.debug(f"ARCHIVE_Q:")
                ...
                q = self._build_retrieve_query(self.DELETE_Q, self.join_columns, need_updates, self.right_table_name)
                logger.debug(f"DELETE FROM RIGHT TABLE Q:")
                result = super()._submit_single_q(q, commit=False)

                # Insert the new row, commit the transaction
                q = self.INSERT_Q.format(table_name=self.right_table_name)
                logger.debug(f"INSERT NEW ROW TO RIGHT TABLE Q:")
    ...
    def process_not_found(self, not_found):
        ...
        for c, batch in enumerate(super()._batcher(not_found)):
            logger.info(f"INSERT batch {c+1}, length: {len(batch)}")
            ...
            self._perform_xform(left, self.xform_left)
            new_records = self._col_subset(self.insert_new_columns, left)
            
            if new_records:
                q = self.INSERT_Q.format(table_name=self.right_table_name)
                logger.debug(f"INSERT NEW ROW INTO RIGHT TABLE Q: {q}")
    ...
    def delta_table(self, do_updates, do_inserts):
        ...
        # Return a list of rows that are found in the right table, and a list that are not found.
        # These rows are chosen based on the join_columns.
        found, not_found = self._delta_q(q)
        logger.info("Delta complete.")
