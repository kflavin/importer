...
def handler(event, context):
    ...
    """
    print("Download GudID files")
    print(find_gudid_zip())
...
def find_gudid_zip():
    html = urlopen(gudid_download_url).read()
    soup = BeautifulSoup(html, 'html.parser')
    links = soup.select("a[download^=gudid_full_release]")
    return links[0].get('href')

def find_ndc_zip():
    html = urlopen(ndc_download_url).read()
    soup = BeautifulSoup(html, 'html.parser')
    links = soup.select("a[href$=ndctext.zip]")
    return links[0].get('href')

def find_orange_zip():
    html = urlopen(orange_download_url).read()
    soup = BeautifulSoup(html, 'html.parser')
    links = soup.select("a[href$=.zip]")
    return orange_base_url + links[0].get('href')
...
def find_indications_zip():
    html = urlopen(indications_download_url).read()
    soup = BeautifulSoup(html, 'html.parser')
    links = soup.select("base[href^=_w]")
    _w = links[0].get('href').rstrip("/")
    # return orange_base_url + links[0].get('href')
...
if __name__ == "__main__":
    ...
    # newlinks = sorted(links, key=lambda x: x['download'], reverse=False)
    ...
    #     print(link.get('href'))
    # print(find_gudid_zip())
    # print(find_orange_zip())
    find_indications_zip()
