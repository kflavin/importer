...
export loader_stage_db_schema=$(aws ssm get-parameters --names "/importer/$LOADER_ENVIRONMENT/stage_db_schema" --region us-east-1 --with-decryption --query Parameters[0].Value --output text)
echo "Loading into $loader_db_schema"
...
export loader_stage_db_schema=$(aws ssm get-parameters --names "/importer/$LOADER_ENVIRONMENT/db_schema" --region us-east-1 --with-decryption --query Parameters[0].Value --output text)
echo "Loading into $loader_db_schema"
time bash RxNorm_Loader_run.sh --context_param contextName="$LOADER_ENVIRONMENT" 2>&1 --context_param endAt=sp